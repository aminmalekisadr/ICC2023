...
from src.utils import score, voting
...
def main():
    ...
    df_label = pd.read_csv(data_parameters['label_file'])
    ...
    Satlite = [SMAP if sys.argv[2] == 'SMAP' else MSL][0]
    ...
    for name in Satlite:
        ...
        data = preprocess.build_df(np.concatenate([train_np, test_np]), start=0)
        import pdb

        Counter1 = data['value'].unique().tolist()
        if len(Counter1) < 50:
            continue
        ...
        dict_ensemble = evaluate_ga('/csv/' + name + '.csv', label_data)
